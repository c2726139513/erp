...
## EdgeOne Pages 配置文件（可选）

如果需要自定义构建配置，可以在项目根目录创建 `edgeone.json` 文件：
...
**注意**：对于大多数 Next.js 项目，EdgeOne Pages 会自动检测并配置，通常不需要创建 `edgeone.json` 文件。
...
EdgeOne Pages 提供 Node Functions 功能，允许在 `node-functions` 目录下创建额外的后端函数。但对于本项目：

- **Next.js API Routes 已足够**，不需要使用 Node Functions
- Node Functions 适用于需要 Express/Koa 框架或更多控制的场景
- 如果未来需要，可以在 `node-functions` 目录下创建函数
...
```bash
curl -X POST https://your-app.edgeone.run/api/users \
...
## 与 Vercel External DB 分支的区别

| 特性 | Vercel External DB | EdgeOne Pages |
|------|-------------------|---------------|
| 部署平台 | Vercel | 腾讯云 EdgeOne Pages |
| 国内访问速度 | 较慢 | 快（国内节点） |
| 域名备案 | 不需要 | 不需要 |
| Next.js 支持 | 完全支持 | 完全支持 |
| API Routes | 完全支持 | 完全支持 |
| 数据库管理 | 手动管理 | 手动管理 |
| 配置复杂度 | 简单 | 简单 |
| 数据独立性 | 完全独立 | 完全独立 |
| 成本 | 有免费额度 | 公测期间免费 |
...
选择建议：
- **Vercel External DB**：面向全球用户，需要 Vercel 生态
- **EdgeOne Pages**：主要面向国内用户，需要更快的访问速度
